...
def get_inputs(config_file,util):
    ...
    global password
    if hasattr(sys.modules[__name__], '_child_pwd') and _child_pwd:
        password = _child_pwd
    elif ('DBA_Pwd' in data) :
...
# Helper function to get valid encoding with fallback for unsupported encodings like CESU-8
def get_valid_encoding(encoding_name):
    """
    Get a valid Python encoding for internal tracking files (iq_tables.list,
    ExtractedTables.out, etc.) that store Unicode strings from pyodbc.
    Falls back to UTF-8 when Python has no codec for the charset name.
    """
    if not encoding_name:
        return 'utf-8'
    try:
        'test'.encode(encoding_name)
        return encoding_name
    except LookupError:
        return 'utf-8'

def get_data_file_encoding():
    """
    Return the charset to pass as dbisql READ ENCODING for data load .sql
    files generated by form_load_table_stmt. These files contain Unicode
    table names from pyodbc, written with get_valid_encoding(charset).
    For charsets Python can encode natively, this equals the DB charset.
    For EUC-TW (no Python codec), files are UTF-8 so dbisql must read UTF-8.
    """
    if not charset:
        return 'UTF-8'
    if charset.upper() in ('EUC_TAIWAN', 'EUC-TW'):
        return 'UTF-8'
    return charset

def get_encoding_errors():
    """
    Return the error handler to use with open() for the current charset.
    For CESU-8, returns 'surrogatepass' so surrogate pairs are preserved.
    For other charsets, returns 'strict' (Python default).
    """
    if not charset:
        return 'strict'
    if charset.upper() == "CESU-8":
        return 'surrogatepass'
    return 'strict'

def decode_cesu8(byte_data):
    """
    Decode CESU-8 encoded bytes into a Python string.
    CESU-8 encodes supplementary characters (U+10000+) as surrogate pairs.
    Uses surrogatepass to keep surrogate pairs intact in the Python string,
    so they can be written back as valid CESU-8 later.
    """
    return byte_data.decode('utf-8', 'surrogatepass')

def encode_cesu8(text):
    """
    Encode a Python string to CESU-8 bytes.
    Supplementary characters (U+10000+) are split into UTF-16 surrogate pairs,
    each encoded as a 3-byte UTF-8 sequence (6 bytes total).
    Handles both proper Unicode chars (from pyodbc) and existing surrogates.
    """
    out = bytearray()
    for ch in text:
        cp = ord(ch)
        if cp > 0xFFFF:
            # Split into surrogate pair, encode each half as 3-byte UTF-8
            high = 0xD800 + ((cp - 0x10000) >> 10)
            low  = 0xDC00 + ((cp - 0x10000) & 0x3FF)
            out.extend(chr(high).encode('utf-8', 'surrogatepass'))
            out.extend(chr(low).encode('utf-8', 'surrogatepass'))
        else:
            out.extend(ch.encode('utf-8', 'surrogatepass'))
    return bytes(out)

def to_cesu8_str(text):
    """
    Convert a Python string so that supplementary characters (U+10000+) are
    replaced with their UTF-16 surrogate pair equivalents. The resulting string
    can then be written with encoding='utf-8', errors='surrogatepass' and will
    produce valid CESU-8 bytes on disk.
    """
    if charset.upper() != "CESU-8":
        return text
    result = []
...
def _schema_file_encoding():
    """
    Return the Python encoding for reading/writing schema files produced by
    iqunload (reload.sql, AutoUpdated_Reload.sql). For charsets Python cannot
    handle natively (EUC-TW, CESU-8), uses a safe fallback.
    """
    upper = charset.upper()
    if upper in ('EUC_TAIWAN', 'EUC-TW'):
        # latin-1 is byte-transparent: every 0x00-0xFF byte round-trips
        # unchanged, so EUC-TW encoded bytes are preserved as-is.
        return 'latin-1'
    return get_valid_encoding(charset)

def read_file_decoded(filepath):
    """
    Read a schema file produced by iqunload in the database charset.
    For CESU-8, reads in binary mode with surrogate-safe handling.
    For EUC-TW (no Python codec), uses latin-1 byte-transparent passthrough.
    For other charsets, uses the charset directly.
    """
    if charset.upper() == "CESU-8":
        with open(filepath, "rb") as f:
            return decode_cesu8(f.read())
    else:
        with open(filepath, "r", encoding=_schema_file_encoding()) as f:
            return f.read()

def write_file_encoded(filepath, content, mode="w"):
    """
    Write a schema file in the database charset. For CESU-8, writes in binary
    mode to preserve surrogate pairs. For EUC-TW, uses latin-1 passthrough.
    For other charsets, uses the charset directly.
    """
    if charset.upper() == "CESU-8":
        bmode = "wb" if mode == "w" else "ab"
        with open(filepath, bmode) as f:
            f.write(encode_cesu8(content))
    else:
        with open(filepath, mode, encoding=_schema_file_encoding()) as f:
            f.write(content)
